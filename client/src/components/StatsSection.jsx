  ...
  const stats = [
    ...
    },
    {
      ...
      value:
        "PKR " +
        (
          bookings.reduce((a, b) => a + Number(b.totalAmount || 0), 0) /
          1000000
        ).toFixed(4) +
        "M",
      icon: Wallet,
  ...
  return (
    <div className="grid grid-cols-2 xl:grid-cols-4 gap-3 mb-6">
      {stats.map((s) => (
        <div
          key={s.label}
          className="bg-white rounded-2xl p-4 border border-green-100 shadow-sm flex items-center gap-3"
        >
          <div className={`${s.bg} p-2.5 rounded-xl shrink-0`}>
            <s.icon size={18} className={s.color} />
          </div>
          <div className="min-w-0">
            <div className="text-xl font-bold text-green-900 truncate">{s.value}</div>

...
export default function StatsSection({ bookings = [] }) {
  ...
  // - Finished / Completed -> Picks full booking `totalAmount`
  // - Confirmed / Pending / Cancelled -> Picks `advancePaid`
  // - Others -> 0
  ...
  const filteredPaymentRevenue = bookings
    .filter((b) => {
      ...
      const bank = (b.bankName || b.bank_name || "").toLowerCase();
      const target = selectedPaymentMethod.toLowerCase();

      const isHabib =
        method.includes("habib") ||
        method.includes("hbl") ||
        bank.includes("habib") ||
        bank.includes("hbl") ||
        bank.includes("metro");
